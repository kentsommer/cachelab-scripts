...
if (PAPI_query_event(papi_events[0]) == PAPI_OK) { \
...
if (PAPI_query_event(papi_events[1]) == PAPI_OK) { \
...
#define PERF_EXIT \
        ...
        fprintf(stderr, "L1 D cache misses:       %ld\n", CtrValues[0]);	\
        fprintf(stderr, "L2 (total) cache misses: %ld\n", CtrValues[1]); 
//        printf("D TLB misses: %ld\n", CtrValues[1]);
//#endif
